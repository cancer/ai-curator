...
- 関心軸のシード文・監視対象（リポジトリ/著者/タグ）は実運用では KV 等の外部ストアに置き、コードから分離する
- 理由は運用面（秘匿ではない）: 関心プロファイルは実運用で頻繁に更新される（フィードバック還流 BS L38）ため、コード変更を伴わず差し替えられるべき可変データである。コードに直書きするとプロファイル更新のたびにデプロイが要る

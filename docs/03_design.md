...
LLM は Workers AI を使用（確定 2026-07-06）。PoC 実測（`docs/poc_results.md` ④）で llama-3.2-3b の日本語品質が不十分と判明したため、実運用前に日本語品質の高い生成モデルへ再選定する。**LLM 要約は全件生成に確定**（2026-07-09。コストが制約でないため体験優先）。なお digest モデルを切り替えても、既存の要約を持つ記事は再要約されない（summaries は article_id で 1 記事 1 行。要約段は行が無い記事のみを対象とし、書き込みも ON CONFLICT DO NOTHING のため）。新モデルで作り直すには該当行の削除が必要。

...
6. **全件の要約生成**（フィードに載る全記事）:
   ...
   - LLM 呼び出し: `env.AI.run(config.digest.model, { messages, max_tokens: 4000, temperature: 0, stream: true })`。digest モデルは推論モデル（Qwen3）で、推論生成が長引くと同期呼び出しでは ~60s ゲートウェイに達し 504 になりうるため **ストリーミング**で呼ぶ。SSE を読み、可視回答（`response` または `choices[].delta.content`）だけを連結し、推論チャンクは破棄する。推論だけで上限に達し可視回答が空になった場合はエラー扱い（リトライ→最終的に当該記事を失敗計上）。`max_tokens` は推論＋回答の合計トークン上限で、Qwen3 実測（完走 ~1,000〜1,100 tok）に余裕を足した値。リンク先HTMLは `<main>` を優先抽出し、無ければページ全体へフォールバックする。system プロンプト内で、(1) 原文と同じ言語のまま、固有名詞・数値・発言者への帰属・複数トピックを保持して要約案を作る、(2) その要約案だけを日本語へ翻訳する、の順に処理する。翻訳時に妥当な日本語訳へ確信を持てない専門用語は原文表記のまま残す。中間要約は出力せず、最終出力は「想定対象読者（1文）」「全体の要約（8〜12文）」「命題（1〜2文）」「結論（1〜2文。統一的な結論がなければ定型文で明記）」の4項目。 user: `タイトル: {title}\n\n本文抜粋:\n{body の先頭 20,000 字}`
...
- **digest LLM モデルの選定（2026-07-14 比較 → 2026-07-15 確定）**: 入出力とも $1未満/M の現行 Workers AI テキスト生成モデルを実記事で比較。当初は品質重視で `@cf/google/gemma-4-26b-a4b-it` を採用したが、**本番初回で約 50% が「推論だけで max_tokens を食い潰し可視回答が空（finish_reason=length）」の暴走**を起こし、summarize step が枠超過→後続 trends step を巻き添えにして落ちた。原因は Gemma / GLM‑4.7 / GPT‑OSS いずれも built-in thinking を止める手段が Workers AI に無いこと（暴走はモデル特性で max_tokens 調整では消えない）。**検討した推論モデルで暴走しないのは Qwen3 30B A3B（`@cf/qwen/qwen3-30b-a3b-fp8`）だけ**（全試行・本番で必ず stop・非空に収束）のため、これを初期値に確定。網羅性は Gemma 等より劣るが「毎日確実に出る」を優先。**llama-3.2-3b-instruct は日本語品質不十分のため選ばない**（前提知識 10）。モデル id は `SYSTEM_CONFIG.digest.model`（コード内固定・UI 非公開）

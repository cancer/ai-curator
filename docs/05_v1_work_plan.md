...
6. **全件の要約生成**（フィードに載る全記事）:
   ...
   - LLM 呼び出し: `env.AI.run(config.digest.model, { messages, max_tokens: 10000, temperature: 0, stream: true })`。digest モデルは推論モデル（Gemma 4）で、同期呼び出しだと推論生成が ~60s ゲートウェイに達し 504 になるため **ストリーミング必須**。SSE を読み、可視回答（`choices[].delta.content` または `response`）だけを連結し、推論チャンクは破棄する。推論だけで上限に達し可視回答が空になった場合はエラー扱い（リトライ→最終的に当該記事を失敗計上）。`max_tokens` は推論＋回答の合計トークン上限で、非決定的な推論量に安全余裕を足した値。リンク先HTMLは `<main>` を優先抽出し、無ければページ全体へフォールバックする。system プロンプト内で、(1) 原文と同じ言語のまま、固有名詞・数値・発言者への帰属・複数トピックを保持して要約案を作る、(2) その要約案だけを日本語へ翻訳する、の順に処理する。翻訳時に妥当な日本語訳へ確信を持てない専門用語は原文表記のまま残す。中間要約は出力せず、最終出力は「想定対象読者（1文）」「全体の要約（8〜12文）」「命題（1〜2文）」「結論（1〜2文。統一的な結論がなければ定型文で明記）」の4項目。 user: `タイトル: {title}\n\n本文抜粋:\n{body の先頭 20,000 字}`
...
- **digest LLM モデルの選定（2026-07-14 実施済み）**: 入出力とも $1未満/M の現行 Workers AI テキスト生成モデルを実記事で比較し、`@cf/google/gemma-4-26b-a4b-it`（Gemma 4 26B A4B）を初期値に採用。理由は、帰属・原語保持を含む要約品質が候補中で高いこと。Qwen3 は同期のまま動くが非決定性が強く網羅性が中程度、GLM/GPT‑OSS は網羅性は高いがコスト・レイテンシで不利。**制約**: Gemma は推論モデルのため同期 `AI.run` では 60s ゲートウェイで 504 になり、ストリーミング必須。かつ推論量が非決定的で、空回答時のリトライにより 1 記事あたり数分に達しうる（Workflow 前提なので許容）。**llama-3.2-3b-instruct は日本語品質不十分のため選ばない**（前提知識 10）。モデル id は `SYSTEM_CONFIG.digest.model`（コード内固定・UI 非公開）
